...
export class IfcService {
  ...
  async loadIfc() {
    await this.ifcViewer?.loadIfcUrl("../assets/model/01.ifc", true);
    await this.retriveAllPlans();
  }

  async retriveAllPlans() {
    await this.ifcViewer?.plans.computeAllPlanViews(0);
    console.log(this.ifcViewer?.plans.planLists)
  }

  selectPlan(idPlan: string, view3d: boolean) {
    if(this.ifcViewer)
      this.ifcViewer.plans.active = view3d;
    this.ifcViewer?.plans.goTo(0, idPlan);
  }

  exitPlanView() {
    if(this.ifcViewer)
    this.ifcViewer.plans.exitPlanView();
  }
